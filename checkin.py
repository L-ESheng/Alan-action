...
if object['code'] == 200:
    print(f">>>@{logindata['phone']}<<<登录成功！")
else:
    print(f"!!!>>>{logindata['phone']}<<<!!!登录失败！请检查密码是否正确！" + str(object['code']))
    exit(object['code'])

res = s.post(url=url2, data=protect('{"type":0}'), headers=headers)
object = json.loads(res.text)
if object['code'] != 200 and object['code'] != -2:
    print(f"!!!>>>{logindata['phone']}<<<!!!签到时发生错误：" + object['msg'])
else:
    if object['code'] == 200:
        print(f">>>@{logindata['phone']}<<<签到成功，经验+" + str(object['point']))
    else:
        print(f"!>>>{logindata['phone']}<<<!重复签到")
time.sleep(1)
print(f"正在为>@{logindata['phone']}<执行歌单遍历任务！")
res = s.post(url=url3,
